...
> 优先级更新（2026-09-07）：用户已明确选择“学习理解优先”。0.7 开发预览采用底层逻辑、定性分析、定量分析（板书）、概念关联与总结学习卡片，主动回忆可选且默认关闭，不包含间隔复习；见[学习工作台发布说明](releases/v0.7.0-beta.1.md)。下文保留此前调查方向的论证；其中 Verify 优先的路线与旧版能力清单不再作为当前实施顺序或现状依据。
