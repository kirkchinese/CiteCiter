...
export function readOnlyCommandStatus(agent) {
...
export async function requireReadOnlyCommand(agent, timeoutMs = 15_000) {
    const initial = readOnlyCommandStatus(agent);
    if (initial.kind === 'ready')
        return;
    if (initial.kind === 'error')
        throw new Error(`read-only switch failed: ${initial.message}`);
    await new Promise((resolve, reject) => {
        ...
        const check = () => {
            ...
            if (status.kind === 'ready')
                ...
                finish(new Error(`read-only switch failed: ${status.message}`));
        };
