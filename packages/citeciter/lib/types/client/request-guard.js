...
async function claimRequestIntent(namespace, identity) {
    ...
    if (stored !== null && stored.startsWith('topic-'))
 ...
 * @returns the pending intent key and request ID.
 */
export function claimAskIntent(topicSessionId, question) {
 ...
 */
export function completeRequestIntent(intent) {
    memoryIntents.delete(intent.key);
    try {
        const storageKey = STORAGE_PREFIX + intent.key;
        if (sessionStorage.getItem(storageKey) === intent.requestId)
