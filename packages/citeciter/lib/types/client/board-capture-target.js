 ...
 */
export function findVisibleCaptureBoard(sessionId, revision, fallback) {
    return [...document.querySelectorAll('[data-citeciter-board]')].find(node => {
        if (fallback.contains(node) || node.dataset.citeciterBoard !== sessionId || node.dataset.boardRevision !== String(revision))
            return false;
