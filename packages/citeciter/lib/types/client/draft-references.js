...
export function topicDraftReferences(topic, documentTitle) {
    ...
    if (topic.documentId !== null)
        references.push({ id: `document:${topic.sessionId}:${topic.documentId}`, kind: 'source', label: '来源文档', content: documentTitle ?? topic.documentId, address: `dsh://document/${encodeURIComponent(topic.documentId)}` });
    if (topic.citation !== null)
        references.push({ id: `excerpt:${topic.sessionId}`, kind: 'excerpt', label: '引用文段', content: topic.citation.displayText });
    return references;
}
...
export function draftReferenceName(reference) {
    if (reference.label !== '来源文档' || reference.address?.endsWith(`/${encodeURIComponent(reference.content)}`))
...
export function serializeDraftReferences(question, references) {
...
export function parseSentReferences(text) {
    const marker = /(?:^|\n\n)【附件：(来源对话|来源文档|引用文段|板书引用)】\n/gu;
    const matches = [...text.matchAll(marker)];
    if (matches.length === 0)
        return { question: text, references: [] };
