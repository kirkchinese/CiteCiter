...
function offsets(node) {
    ...
    if (start === undefined || end === undefined)
        throw new Error('Markdown parser omitted source offsets');
    return [start, end];
}
...
function inlineCodeUnits(node, markdown) {
    ...
    if (visibleText(units) !== value && units[0]?.text === ' ' && units.at(-1)?.text === ' ') {
        ...
        if (interior.some((unit) => unit.text !== ' '))
            units = interior;
    }
    return visibleText(units) === value ? units : [];
}
...
function codeUnits(node, markdown) {
    ...
    if (valueLines.length > lines.length)
        return [];
    const units = [];
    ...
    if (lines.slice(valueLines.length).some((line) => line.text.trim() !== ''))
        return [];
    return units;
}
function imageAltUnits(node, markdown) {
    ...
    for (let cursor = 2; cursor < source.length; cursor++) {
        ...
        depth--;
        if (depth === 0)
            return decodedUnits(markdown, node.alt ?? '', start + 2, start + cursor);
    }
    return [];
}
...
function trimMapped(units) {
    ...
    while (start < end && isWhitespace(units[start]))
        start++;
    while (end > start && isWhitespace(units[end - 1]))
        end--;
...
function sourceRange(units, start, end) {
    ...
    if (first === undefined || last === undefined || first.startOffset >= last.endOffset)
        return null;
...
function candidatesIgnoringSyntheticWhitespace(markdown, projection, needle) {
    ...
    for (const unit of projection)
        visibleOffsets.push(visibleOffsets.at(-1) + unit.text.length);
    const candidates = [];
    const seen = new Set();
    for (let start = 0; start < projection.length; start++) {
        ...
        while (needleOffset < needle.length && unitIndex < projection.length) {
            ...
            if (unit.synthetic === true) {
                while (projection[unitIndex]?.synthetic === true)
                    unitIndex++;
                needleOffset += /^\s+/u.exec(needle.slice(needleOffset))?.[0].length ?? 0;
                continue;
            }
            if (!needle.startsWith(unit.text, needleOffset))
                break;
            needleOffset += unit.text.length;
            lastIndex = unitIndex;
            unitIndex++;
        }
        if (needleOffset !== needle.length || lastIndex < start)
            continue;
 ...
 */
export function markdownSourceCandidates(markdown, displayText) {
