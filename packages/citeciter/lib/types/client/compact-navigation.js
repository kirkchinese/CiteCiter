    ...
    useEffect(() => {
        ...
        const cover = () => {
            ...
            for (const element of frame.children) {
                if (!(element instanceof HTMLElement) || element.hasAttribute('data-shell-overlay') || owned.has(element))
                    continue;
                const rect = element.getBoundingClientRect();
                if (rect.width <= 0 || rect.height <= 0 || rect.right <= bounds.left + 1 || rect.bottom <= bounds.top + 1)
                    continue;
                if (element.classList.contains('dshDesktopWindowsCaptionRow') || element.classList.contains('dshDesktopMacCaptionRow'))
                    continue;
        ...
        return () => {
            ...
            for (const [element, previous] of owned) {
                ...
                if (previous.hidden === null)
                    element.removeAttribute('aria-hidden');
                else
                    element.setAttribute('aria-hidden', previous.hidden);
