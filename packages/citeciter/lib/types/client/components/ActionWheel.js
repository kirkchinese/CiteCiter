    ...
    return _jsxs(OverlayPortal, { children: [wheel !== null && _jsxs("div", { className: css.wheel, style: { left: wheel.x, top: wheel.y, '--wheel-scale': wheel.scale }, "data-citeciter-menu": true, "data-citeciter-wheel": true, ref: menu, tabIndex: -1, role: "menu", "aria-label": "CiteCiter \u9009\u6587\u52A8\u4F5C", onKeyDown: event => {
                    if (event.key === 'Escape')
                        ...
                        return _jsxs("button", { className: css.slot, type: "button", role: "menuitem", "aria-disabled": slot === null, "data-active": wheel.active === index || undefined, style: { left: 180 + Math.cos(angle) * 114, top: 180 + Math.sin(angle) * 114 }, onMouseEnter: () => { if (!wheel.held)
                                actions.focus(index); }, onFocus: () => actions.focus(index), onClick: () => actions.choose(index), title: slot?.prompt, children: [_jsx("span", { className: css.slotNumber, children: index + 1 }), _jsx("strong", { children: slot?.label ?? '空槽' }), _jsx("small", { children: slot === null ? '在设置中添加' : slot.ask ? '需输入 · 选模型' : `准备草稿 · ${slot.presentation === 'side' ? '侧边' : '悬浮'}` })] }, index);
                    }), _jsxs("button", { type: "button", className: css.center, onClick: actions.cancel, "aria-label": "\u53D6\u6D88\u8F6E\u76D8", children: ["\u53D6\u6D88", _jsx("small", { children: "Esc" })] }), _jsx("div", { className: css.caption, role: "status", children: active == null ? '移向动作 · 回到中心取消' : `${active.label} · ${active.ask ? '松开后输入问题并选择模型' : '松开后准备草稿'}` })] }), pending !== null && _jsxs("form", { className: css.prompt, "data-citeciter-menu": true, role: "dialog", "aria-label": `${pending.action.label}：输入问题`, style: { '--prompt-x': `${pending.x - 210}px`, '--prompt-y': `${pending.y - 100}px` }, onSubmit: event => { event.preventDefault(); void actions.submit(); }, children: [_jsxs("header", { children: [_jsx("strong", { children: pending.action.label }), _jsx("button", { type: "button", onClick: actions.cancel, "aria-label": "\u5173\u95ED\u63D0\u95EE", children: "\u00D7" })] }), _jsx("blockquote", { children: actionSourceQuote(pending.source).slice(0, 180) }), pending.action.ask && _jsx("textarea", { autoFocus: true, "aria-label": "\u8865\u5145\u95EE\u9898", placeholder: "\u8F93\u5165\u4F60\u7684\u95EE\u9898\u2026", maxLength: 7500, rows: 3, value: state.question, disabled: state.submitting, onChange: event => actions.setQuestion(event.currentTarget.value), onKeyDown: event => { if (event.key === 'Enter' && (event.ctrlKey || event.metaKey) && !event.nativeEvent.isComposing) {
                            event.preventDefault();
                            void actions.submit();
                        } } }), pending.action.ask && _jsx("div", { className: css.slotActions, children: (snapshot.settings.promptTemplates ?? []).map(template => _jsx("button", { type: "button", disabled: state.submitting, onClick: () => actions.setQuestion(template.text), children: template.label }, template.id)) }), (pending.action.ask || state.error !== null) && _jsx(ModelChoice, { providers: snapshot.providers, value: state.model, onChange: actions.setModel, disabled: state.submitting }), state.error !== null && _jsx("p", { role: "alert", className: css.error, children: state.error }), _jsxs("footer", { children: [_jsx("span", { children: pending.action.presentation === 'side' ? '在学习栏中打开' : '在悬浮窗中打开' }), _jsx("button", { type: "submit", disabled: state.submitting || pending.action.ask && state.question.trim() === '', children: state.submitting ? '正在创建…' : state.error === null ? '准备草稿' : '重试' })] })] })] });
}
