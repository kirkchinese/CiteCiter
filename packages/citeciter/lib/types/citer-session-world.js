...
import { CiterAgentRegistry } from "./citer-agent-registry.js";
import { createCiterSessionStore } from "./citer-session-store.js";
import { loadHostAgentModules } from "./host-agent-modules.js";
...
export class CiterSessionWorld {
    ...
    async start(host, root, access) {
        ...
        this.release = host.effect(function* () {
            ...
            yield async () => {
                ...
                if (errors.length > 0)
                    failure = new AggregateError(errors, 'Citer Agent drain failed');
            };
    ...
    dispose() {
        return this.disposal ??= (async () => {
            ...
            await this.release?.();
            while (this.fiber?.inertia !== undefined)
                await this.fiber.inertia;
        })();
    }
}
