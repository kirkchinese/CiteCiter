...
export class SourceStorage {
    ...
    async root(sourceSessionId, create = false) {
        ...
        try {
            ...
            if (this.pending.get(sourceSessionId) === operation)
                this.pending.delete(sourceSessionId);
        }
    }
    async resolveRoot(sourceSessionId, create) {
        ...
        if (!info.isDirectory() || info.isSymbolicLink())
            throw new Error('Citer 拒绝使用链接或非目录的来源存储路径');
        const marker = resolve(directory, 'owner.json');
        const markerInfo = await lstat(marker).catch(error => { if (absent(error))
            return undefined; throw error; });
        if (markerInfo !== undefined && (!markerInfo.isFile() || markerInfo.isSymbolicLink()))
            throw new Error('Citer 来源标记必须是普通文件');
        let raw = await readFile(marker, 'utf8').catch(error => { if (absent(error))
            return undefined; throw error; });
        if (raw === undefined) {
            if (!create)
                return undefined;
            if ((await readdir(directory)).length !== 0)
                throw new Error('来源 citeciter 目录已有未识别内容，未覆盖');
        ...
        if (owner.sourceSessionId !== sourceSessionId)
