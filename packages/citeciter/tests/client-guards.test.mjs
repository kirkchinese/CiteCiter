...
test('settled reasoning-only and mixed assistant calls remain citable during an open turn', () => {
  ...
  withConversationDom(({ body, setRange }) => {
    ...
    const collapsedAssistant = new FakeElement('article', { chatFlowKind: 'assistant-step', chatAnchorKey: 'assistant:9' })
    const collapsedReasoning = new FakeElement('div', { variant: 'think' })
    const collapsedHeaderText = text('Think Need the source.')
    const collapsedHeader = new FakeElement('div', { disclosureRow: '' }).append(collapsedHeaderText)
    const collapsedAnswerText = text('Final answer.')
    const collapsedAnswer = new FakeElement('p').append(collapsedAnswerText)
    collapsedReasoning.append(collapsedHeader)
    collapsedAssistant.append(collapsedReasoning, collapsedAnswer)
    body.append(collapsedAssistant)

    const committed = 'Need the source.\n\nFinal answer.'
    setRange(new FakeRange(collapsedHeaderText, 0, collapsedAnswerText, 5, 'Think Need the source. Final'))
    const collapsedMixedEvent = contextMenu(collapsedAnswer)
    const collapsedMixed = claimSelectionContextMenu(collapsedMixedEvent, 'source', committed)
    assert.equal(collapsedMixedEvent.defaultPrevented, true)
    assert.equal(collapsedMixed?.entryId, 'citeciter.entry.assistant')
    assert.equal(collapsedMixed?.sourceHintText, committed)
    assert.equal(collapsedMixed?.startOffset, 0)
    assert.equal(collapsedMixed?.endOffset, committed.length)
    assert.equal(resolveCitationRange(collapsedMixed, committed).sourceText, committed)
  })
})
