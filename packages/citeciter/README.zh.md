...
![0.7 学习工作区结构示意](https://raw.githubusercontent.com/kirkchinese/CiteCiter/codex/learning-workspace-0.7/assets/docs/learning-workspace.svg)
...
![八槽轮盘、提问与模型选择、侧边和悬浮模式](https://raw.githubusercontent.com/kirkchinese/CiteCiter/codex/learning-workspace-0.7/assets/docs/selection-wheel.svg)
...
中心、空槽、轮盘外、Esc、失焦和来源切换取消轮盘。右键短按保留可点击轮盘；方向键切换，Enter 确认，数字 1–8 直接选择。Shift + 右键保留原生菜单。工具结果入口引用整张工具卡；阅读器引用选中文本。
...
提问框打开后，切换应用、调整窗口大小或点击外部不会清空问题与模型；主动关闭、Esc 或切换来源会话才取消草稿。刷新和重启不保留草稿。提交期间阻止重复执行；失败保留问题、模型和来源供重试。已被宿主接受的 Topic 创建不会因此撤销。模式提示词作为普通用户消息写入 Topic 日志。

## 原生文件预览

在 DSH 原生预览的查看方式中选择“CiteCiter 学习”。宿主负责打开、读取和刷新文件；CiteCiter 显示可选取的 UTF-8 源文本。选文后使用轮盘或“选文动作”按钮。支持文本、Markdown 和常见代码文件；开始学习时保存完整快照，后续文件修改不会改写这份来源。原生学习查看方式也使用 500 KiB 分页。标题、链接语法和代码按原文字面匹配；轮盘与提问框可在原生文件全屏时使用。
...
3. 学习栏在“讲解 / 学习卡”间切换。板书统一显示在主区“小黑板”标签中，以画布展示公式、推导和图示的位置关系；侧栏不再重复显示。定量阶段仍在学习栏发送。
4. 选择“总结学习卡片”并发送。模型先检查本 Topic 的结论、板书、成立条件和计算，纠正错误，标明未核实内容，再通过 `learning_cards` 提交完整卡片组。核对在同一轮请求中进行；例子与参考答案也须同步修正。之后可阅读、导出或追问修订。
...
| 概念关联 | 前提、相近概念、区别与应用 |
| 总结学习卡片 | 先核对与纠错，再生成结论、例子、自测问题与参考答案 |
...
| 输入区底部 | 左侧选择思考强度，右侧选择模型并发送；保存模型设置期间暂停发送 |
| 顶部 ···（Topic 设置） | 重命名、归档或删除 |
| 停止 | 停止当前生成，之后可以继续追问 |
...
![来源、Topic 日志与学习输出的数据流](https://raw.githubusercontent.com/kirkchinese/CiteCiter/codex/learning-workspace-0.7/assets/docs/data-flow.svg)
...
模型决定讲解内容与工具调用，阶段按钮不保证模型一定生成板书或卡片。总结前的自检是模型指令，不是独立事实验证；模型仍可能遗漏错误或生成不准确的来源定位。当前没有独立卡片编辑器、跨 Topic 搜索、知识图谱数据库或跨设备同步。

## 本地安装 0.7 开发版

最新版宿主使用本分支的 `0.7.0-beta.3`。稳定 `0.6.0` 对应旧宿主，安装说明保留在 [0.6.0 Release](https://github.com/kirkchinese/CiteCiter/blob/codex/learning-workspace-0.7/docs/releases/v0.6.0.md)。在本分支仓库根目录执行：
...
`test:snapshot` 使用无密钥模型和一次性真实 DSH profile，校验五阶段、板书、卡片、重启恢复和管理操作。确定性测试验证程序行为，不评定模型的教学质量。详细结果和未覆盖项目见 [验收记录](https://github.com/kirkchinese/CiteCiter/blob/codex/learning-workspace-0.7/docs/validation/2026-09-12-acceptance.md)。

开发命令与打包流程见 [贡献指南](https://github.com/kirkchinese/CiteCiter/blob/codex/learning-workspace-0.7/CONTRIBUTING.zh.md)，本分支变更见 [0.7 开发说明](https://github.com/kirkchinese/CiteCiter/blob/codex/learning-workspace-0.7/docs/releases/v0.7.0-beta.3.md)。构建和打包不会发布 npm。
