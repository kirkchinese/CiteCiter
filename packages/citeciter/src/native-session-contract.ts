...
const attachmentId = z.string().min(1).transform(value => value as AttachmentIdType)
...
}).strict().transform(({ name, originalDimensions, ...image }): ImageAttachmentRef => ({ ...image, ...(name === undefined ? {} : { name }), ...(originalDimensions === undefined ? {} : { originalDimensions }) }))
