...
import { canonicalCitationIdentity, type CitationRecord } from './thread.ts'
...
export function validateCitation(agent: Agent, citation: CitationRecord): void {
  ...
  const completedStep = inheritedTail.some((event) => (
    event.type === 'step/end'
    && event.data.turn === anchor.data.turn
    && event.data.step === anchor.data.step
  ))
  const completedTurn = inheritedTail.some((event) => (
    event.type === 'turn/end' && event.data.turn === anchor.data.turn
  ))
