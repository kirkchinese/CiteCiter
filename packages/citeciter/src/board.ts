...
export const BOARD_PROTOCOL_VERSION = 4 as const
...
const boardColorValueSchema = z.string().max(80).regex(
  /^(#[0-9a-fA-F]{3,8}|[a-zA-Z]+|(rgb|rgba|hsl|hsla)\([\d\s,.%()/-]+\))$/u,
)
...
const boardImageContentSchema = z.string().max(200_000).regex(
  /^data:image\/(?:png|jpeg|webp|gif|svg\+xml);base64,[A-Za-z0-9+/]+={0,2}$/u,
)
...
function addEnvelopeIssues(
  value: { readonly x: number, readonly y: number, readonly w: number, readonly h: number },
...
export interface BoardElementState {
  ...
  readonly animation?: { readonly name: 'fade-in' | 'slide-in' | 'pulse' | 'highlight', readonly durationMs: number, readonly iterations: number, readonly run: number } | undefined
}
...
export const EMPTY_BOARD_STATE: BoardState = new Map()

function intersects(
  left: { readonly x: number, readonly y: number, readonly w: number, readonly h: number },
  right: { readonly x: number, readonly y: number, readonly w: number, readonly h: number },
): boolean {
...
function assertElementFits(element: BoardElementState): void {
  boardElementStateSchema.parse(element)
}

function assertBoardBudget(state: BoardState): void {
...
): { readonly state: BoardState, readonly revision: number } {
  if (ops.length === 0) throw new Error('board commit must contain at least one op')
  if (ops.length > BOARD_MAX_BATCH_OPS) throw new Error(`board commit cannot exceed ${BOARD_MAX_BATCH_OPS} ops`)
  ...
  for (const op of ops) {
    switch (op.op) {
      case 'clear':
        ...
        break
      case 'set': {
        const element: BoardElementState = {
          id: op.id,
          kind: op.kind,
          content: op.content,
          x: op.x,
      ...
      case 'update': {
        ...
        if (current === undefined) throw new Error(`cannot update unknown board element ${op.id}`)
        const element: BoardElementState = {
          ...current,
      ...
      case 'animate': {
        const current = next.get(op.id)
        if (current === undefined) throw new Error(`cannot animate unknown board element ${op.id}`)
        next.set(op.id, {
          ...current,
          animation: {
            name: op.animation,
      ...
      case 'focus':
        ...
        for (const [id, element] of next) {
          ...
          if (element.focused !== focused) next.set(id, { ...element, focused })
        }
        break
      case 'remove':
        next.delete(op.id)
      ...
      case 'clear_region':
        for (const [id, element] of next) {
          if (intersects(element, op)) next.delete(id)
        }
        break
      default:
