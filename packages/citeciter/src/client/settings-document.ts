...
const INITIAL: SettingsDocumentSnapshot = Object.freeze({
  status: 'idle',
  opening: false,
  error: null,
  message: null,
...
function memoryStore<Value>(initial: Value): SnapshotStore<Value> {
  let state = initial
 ...
 */
export function createSettingsDocumentController(
  describe: SettingsDescribeFace,
  openDocument: (signal: AbortSignal) => Promise<void>,
): SettingsDocumentController {
  const store: SnapshotStore<SettingsDocumentSnapshot> = memoryStore(INITIAL)
