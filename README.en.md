...
Use `0.7.0-beta.2` with the current host. Stable `0.6.0` targets the older host; see its [release notes](docs/releases/v0.6.0.md). Run these commands from this branch's repository root:
...
`test:snapshot` uses a keyless model in a disposable real DSH profile to verify stages, boards, cards, restart recovery and management operations. Deterministic tests verify software behavior, not teaching quality. See the [acceptance record](docs/validation/2026-09-11-latest.md) for results and coverage limits.

See [Contributing](CONTRIBUTING.md) for development and packaging, and the [0.7 development notes](docs/releases/v0.7.0-beta.2.md) for changes. Building or packing does not publish to npm.
