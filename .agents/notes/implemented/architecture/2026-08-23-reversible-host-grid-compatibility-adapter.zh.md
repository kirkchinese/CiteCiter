...
取代：[Desktop 覆盖抽屉与生命周期所有权](2026-08-23-desktop-overlay-and-lifecycle.zh.md)中的面板布局决定；其中的生命周期、选区、Desktop 目标和验收决定继续有效。
...
CiteCiter 继续把面板注册在 `shell.overlay`。当所属 AppFrame 能同时容纳最小 360 像素的 CiteCiter 和 480 像素主对话时，版本锁定的兼容适配器会在该 frame 上增加一个带 owner 的 namespaced 属性和两个 namespaced CSS 变量，并以 CiteCiter 宽度替换可见的 details 轨。适配器不注册 `details` slot，也不调用 `ctx.layout.closeDetails()`。
...
只使用 overlay 的候选仍可作为公共 API 的 A/B 基线，但不再代表当前候选。本决定之后必须重新生成包与浏览器证据。上游 Discussion 请求由 AppFrame 管理的可逆右侧 dock 扩展点；上游提供该公共扩展后，CiteCiter 应删除此适配器。
